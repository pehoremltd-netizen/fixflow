  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-muted-foreground">Assigned maintenance tasks</p>
      </div>
              ...
              {filtered.filter(wo => wo.status !== "completed").length === 0 ? (
                ...
                filtered.filter(wo => wo.status !== "completed").map((wo, i) => (
                  <motion.div
                    ...
                    className="flex items-center justify-between p-4 border-b last:border-0 hover:bg-accent/50 transition-colors"
                  >
                    <div className="flex items-start gap-3 flex-1">
                      <div className={`flex h-10 w-10 items-center justify-center rounded-lg ${
                        wo.priority === "critical" ? "bg-red-100" :
                        wo.priority === "high" ? "bg-[#E1B000]/10" : "bg-[#D4AF37]/10"
                      }`}>
                        {wo.priority === "critical" ? (
                          <AlertTriangle className="h-5 w-5 text-red-600" />
                        ) : (
                          <Wrench className={`h-5 w-5 ${wo.priority === "high" ? "text-orange-600" : "text-blue-600"}`} />
                        )}
                      </div>
                      <div>
