...
const STORAGE_KEY = "fixflow-upline-managers";

function getItems(): UplineManager[] {
  if (typeof window === "undefined") return [];
  try { const raw = localStorage.getItem(STORAGE_KEY); return raw ? JSON.parse(raw) : []; } catch { return []; }
}
function setItems(data: UplineManager[]) {
...
function seed() {
  if (getItems().length === 0) {
    setItems([
      { id: "um-1", name: "City Properties Inc.", contact: "Robert Johnson", email: "robert@cityprops.com", portal: "cityprops.fixflow.com", status: "active" },
      { id: "um-2", name: "Green Valley Estates", contact: "Linda Green", email: "linda@greenvalley.com", portal: "greenvalley.fixflow.com", status: "active" },
      { id: "um-3", name: "Metro Commercial REIT", contact: "David Kim", email: "david@metroreit.com", portal: "metroreit.fixflow.com", status: "active" },
      { id: "um-4", name: "Summit Property Group", contact: "Sarah Connor", email: "sarah@summitpg.com", portal: "summit.fixflow.com", status: "inactive" },
    ]);
  }
}

export default function UplineManagersPage() {
  const [items, setItemsState] = useState<UplineManager[]>([]);
  ...
  useEffect(() => { seed(); setItemsState(getItems()); setLoading(false); }, []);
  ...
  const handleCreate = (e: React.FormEvent) => {
    ...
    setOpen(false);
    setSubmitting(false);
    setToast("Upline Manager created");
  };
  ...
  const handleDelete = (id: string) => {
    ...
    setItems(updated);
    setItemsState(updated);
    setToast("Upline Manager removed");
  };

  const handleToggleStatus = (id: string) => {
    const updated = items.map((s) => s.id === id ? { ...s, status: s.status === "active" ? "inactive" as const : "active" as const } : s);
    setItems(updated);
    setItemsState(updated);
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-3xl font-bold text-foreground">Upline Manager Management</h1>
          <p className="text-muted-foreground">Manage upline manager portals and access</p>
        </div>
        <Dialog open={open} onOpenChange={setOpen}>
          <DialogTrigger asChild>
            <Button className="gap-2 bg-primary text-primary-foreground hover:bg-primary/90"><Plus className="h-4 w-4" /> Add Upline Manager</Button>
          </DialogTrigger>
          <DialogContent className="border-border bg-card text-foreground sm:max-w-md">
            <DialogHeader><DialogTitle className="text-foreground">Create Upline Manager Portal</DialogTitle></DialogHeader>
      ...
      {loading ? (
        ...
        <p className="text-muted-foreground text-sm py-12 text-center">No upline managers yet.</p>
      ) : (
