...
import { getHealthColor, getHealthBg, getHealthBadgeVariant } from "@/lib/store/ai-intelligence";
import type { AssetHealth } from "@/lib/store/ai-intelligence";
...
export function AssetHealthCard({ data, index = 0 }: AssetHealthCardProps) {
  ...
  const trendColor = data.trend === "improving" ? "text-primary" : data.trend === "degrading" ? "text-destructive" : "text-text-tertiary";
  ...
  return (
    ...
    >
      <Card className="relative overflow-hidden group border-border bg-card card-hover">
        <CardContent className="p-4">
          <div className="flex items-start justify-between mb-3">
            <div className="flex-1 min-w-0">
              <p className="text-sm font-medium text-foreground truncate">{data.assetName}</p>
              <p className="text-xs text-text-tertiary">{data.category}</p>
            </div>
